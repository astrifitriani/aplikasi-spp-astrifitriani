...
if( empty( $_SESSION['iduser'] ) ){
   ...
   if(isset($_REQUEST['submit'])){
      ...
      $sql = mysqli_query($koneksi,"SELECT s.*,p.* FROM siswa s INNER JOIN pembayaran p ON s.nisn = p.nisn AND p.nisn='$nis'");
...
<?php
   ...
   $qsiswa = mysqli_query($koneksi,"SELECT * FROM siswa WHERE nisn='$nis'");
   list($nisn,$nis,$nama,$idprodi) = mysqli_fetch_array($qsiswa);
   
   echo '<div class="row">';
   echo '<div class="col-sm-6"><table class="table table-bordered">';
   echo '<tr><td colspan="2">Nomor Induk</td><td colspan="3">'.$nisn.'</td></tr>';
   echo '<tr><td colspan="2">Nama Siswa</td><td colspan="3">'.$nama.'</td></tr>';
   echo '<tr class="success"><th width="50">#</th><th width="100">NISN</th><th>Tahun</th><th>Bulan</th><th>Tanggal Bayar</th><th>Jumlah</th>';
   echo '</tr>';
   
   //tampilkan histori pembayaran, jika ada
   $qbayar = mysqli_query($koneksi,"SELECT * FROM pembayaran WHERE nisn='$nisn'");
   if(mysqli_num_rows($qbayar) > 0){
      $no = 1;
      while(list($id_pembayaran,$id_petugas,$nisn,$tgl_bayar,$bulan_bayar,$tahun_bayar,$id_spp,$jumlah) = mysqli_fetch_array($qbayar)){
         echo '<tr><td>'.$no.'</td>';
         echo '<td>'.$nisn.'</td>';
         echo '<td>'.$tahun_bayar.'</td>';
         echo '<td>'.$bulan_bayar.'</td>';
