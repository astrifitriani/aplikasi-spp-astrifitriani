...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['sub'] )){
		...
		if(isset($_REQUEST['submit'])){
			...
			$sql = mysqli_query($koneksi,"SELECT password FROM petugas WHERE id_petugas='$iduser' AND password='$pass1'");
			if(mysqli_num_rows($sql) > 0){
				$do = mysqli_query($koneksi,"UPDATE petugas SET password='$pass2' WHERE id_petugas='$iduser'");
				if($sql > 0){
...
<?php
		...
		echo '<tr><td width="130">Nama Lengkap</td><td>: '.$_SESSION['nama_petugas'].'</td></tr>';
